...
contract OMOVault is ERC4626 {
    ...
    function _deposit(uint256 amount, address receiver) external virtual {
        uint256 fee = _feeOnTotal(amount, _entryFeeBasisPoints());
        address recipient = _entryFeeRecipient();
        uint256 netAssets = amount.sub(fee);

        super.deposit(netAssets, receiver);

        if (fee > 0 && recipient != address(this)) {
            SafeERC20.safeTransfer(depositToken, recipient, fee);
        }
        
        depositTimestamp[receiver] = block.timestamp;
        emit Deposit(index, msg.sender, receiver, netAssets, amount);
        index++;
    }

    function processWithdraw(address caller, address receiver, address owner, uint256 assets, uint256 shares) external onlyOwner {
